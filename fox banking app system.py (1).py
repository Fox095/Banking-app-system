...
window.title("Welcome to FoxBank")
...
def fin_sign():
    ...
    for check_name in accounts:
        ...
        if newfile == check_name:
            notifications.config(
                fg='orange', text="Sorry somebody already has that USERNAME!")
            return
            ...
            File.close()
            notifications.config(
                fg='blue', text="Congratulations Your Account is successfully Created")
...
def deposit():
    ...
    deposit_display_screen.geometry("300x250")
...
def fin_deposit():
    ...
    current_rands_label.config(
        text="Your now current balance is:R"+str(new_balance), fg="blue")
    deposit_notifications.config(
        text="Deposit was successfully made", fg="blue")
...
def withdraw():
    ...
    withdraw_display_screen.geometry("300x250")
...
def fin_withdraw():
    if withdraw_rands.get() == "":
        withdraw_notifications.config(
            text="Sorry...you must enter something into the Bank", fg="orange")
        return
    if float(withdraw_rands.get()) <= 0:
        withdraw_notifications.config(
            text="Sorry...you must enter a number greater than 0", fg="orange")
        return
    file = open(LogName + '.csv', 'r+')
    data = file.read()
    info = data.split('\n')
    ...
    if float(new_balance) < 0:
        withdraw_notifications.config(
            text="Sorry...you can not WITHDRAW more than you have", fg="orange")
        return
    data = data.replace(old_balance, str(new_balance))
    file.seek(0)
    file.truncate(0)
    ...
    withdraw_rands_label.config(
        text="You currently have:R "+str(new_balance), fg="blue")
    withdraw_notifications.config(
        text="The withdrawal was successfully processed", fg="blue")
...
Label(window, bg='grey', text="Welcome to FoxBank\n Register to start Banking with US",
      font=(70)).grid(row=0, sticky=N, pady=50, padx=30)

Button(window, bg='white', text="Register", font=(40), width=30,
       height=2, command=Register).grid(row=4, sticky=N, pady=10, padx=120)

Button(window, bg='white', text="Login", font=(40), width=30, height=2,
